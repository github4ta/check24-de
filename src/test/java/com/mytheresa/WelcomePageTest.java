...
public class WelcomePageTest extends AuthPage {
    ...
    @Test
    @DisplayName("UI-TC-006: Validation error when submitting invalid Email format")
    public void invalidEmailFormatTest() throws InterruptedException {
        ...
        List<String> invalidEmailArrayList = new ArrayList<String>();

        invalidEmailArrayList.add("test");
        invalidEmailArrayList.add("test@");
        invalidEmailArrayList.add("@domain.com");
        invalidEmailArrayList.add("test @domain.com");
        invalidEmailArrayList.add("test@ domain.com");
        invalidEmailArrayList.add("test@@domain.com");

        Set<String> invalidEmailSet = new HashSet<String>();

        invalidEmailSet.add(".test@domain.com");
        invalidEmailSet.add("test.@domain.com");
        invalidEmailSet.add("test@domain..com");
        invalidEmailSet.add("test@-domain.com");
        invalidEmailSet.add("test@domain-.com");
        invalidEmailSet.add("test@.com");
        invalidEmailSet.add("test@domain.c");

        Iterator<String> invalidEmailArrayListIterator = invalidEmailArrayList.iterator();
        while (invalidEmailArrayListIterator.hasNext()) {
            String invalidEmail = invalidEmailArrayListIterator.next();
            driver.findElement(getEmailInput()).sendKeys(invalidEmail);
            driver.findElement(getLogInButton()).click();
            Thread.sleep(550);
            softly.assertThat((driver.findElement(getEmailError()).isDisplayed())).isTrue();
            softly.assertThat(driver.findElement(getEmailError()).getText().equals("Invalid email address")).isTrue();

            cleanField(driver, getEmailInput() );
        }

        for (String invalidEmail : invalidEmailSet) {
            driver.findElement(getEmailInput()).sendKeys(invalidEmail);
            driver.findElement(getLogInButton()).click();
            Thread.sleep(550);
            softly.assertThat((driver.findElement(getEmailError()).isDisplayed())).isTrue();
    ...
    public void cleanField(WebDriver driver, By locator) {
